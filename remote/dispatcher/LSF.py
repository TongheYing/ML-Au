...
from remote.dispatcher.Batch import Batch
...
class LSF(Batch):
    ...
    def check_status(self):
        ...
        if job_id == "":
            raise RuntimeError("job %s is has not been submitted" % self.job_id_name)
    ...
    def do_submit(self,
        ...
        if 'task_max' in res and res['task_max'] > 0:
            while self._check_sub_limit(task_max=res['task_max']):  # check whether jobs reached the limit
                time.sleep(60)
        script_str = self.sub_script(job_dirs, cmd, args=args, res=res, outlog=outlog, errlog=errlog)
        self.context.write_file(self.sub_script_name, script_str)  # write the job script of LSF system
    ...
    def _check_sub_limit(self, task_max, **kwarg):
        ...
        if (njobs_run + njobs_pend) < task_max:
            return False
        else:
            return True
