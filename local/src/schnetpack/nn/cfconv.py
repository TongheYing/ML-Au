...
from local.src.schnetpack.nn import Dense
from local.src.schnetpack.nn.base import Aggregate
...
class CFConv(nn.Module):
    ...
    def forward(self, x, r_ij, neighbors, pairwise_mask, f_ij=None):
        ...
        """
        if f_ij is None:  # f_ij是扩展后的高斯距离，它的shape是(100, 9, 8, 20)
            f_ij = r_ij.unsqueeze(-1)

        # pass expanded interactomic distances through filter block
        W = self.filter_network(f_ij)  # 普通的全连接层，返回的W的shape是(100, 9, 8, 128)
        # apply cutoff
        if self.cutoff_network is not None:
            C = self.cutoff_network(r_ij)  # C的shape是(100, 9, 8)
            W = W * C.unsqueeze(-1)  # unsqueeze在最后一个维度增加了一个维度，即现在C的shape是(100, 9, 8, 1)

        # pass initial embeddings through Dense layer
        y = self.in2f(x)  # x的shape是(100, 9, 128)，self.in2f是个普通的全连接层
        # reshape y for element-wise multiplication by W
        nbh_size = neighbors.size()  # neighbors的shape是(100, 9, 8)
