...
from local.src.schnetpack.nn.initializers import zeros_initializer
...
class GetItem(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        return inputs[self.key]  # 取得self.key即representations的输出值，即SchNet层的输出值
...
class ScaleShift(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        y = input * self.stddev + self.mean  # 反向denorm
        return y
...
class Aggregate(nn.Module):
    ...
    def forward(self, input, mask=None):
        ...
        # mask input
        if mask is not None:
            input = input * mask[..., None]  # mask的shape是(100, 9, 8)，而input的shape是(100, 9, 8, 128)
