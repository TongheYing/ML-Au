...
def atom_distances(
    ...
    # Get atomic positions of all neighboring indices, positions的shape是(100, 9, 3)，neighbors的shape是(100, 9, 8)
    pos_xyz = positions[idx_m, neighbors[:, :, :], :]  # shape是(100, 9 ,8, 3)，获得了每个原子相邻原子的坐标

    # Subtract positions of central atoms to get distance vectors
    dist_vec = pos_xyz - positions[:, :, None, :]  # dist_vec的shape是(100, 9, 8, 3)
    ...
    if cell is not None:
        ...
        offsets = offsets.view(B, A, N, D)  # 对于非周期性结构，即cell=[0, 0, 0]，offsets = 0
        dist_vec += offsets

    # Compute vector lengths
    distances = torch.norm(dist_vec, 2, 3)  # distances的shape是(100, 9, 8)

    if neighbor_mask is not None:  # 将cutoff内个数不够的位置填充满, neighbor_mask的shape是(100, 9, 8)且全为1.0
        ...
        tmp_distances[neighbor_mask != 0] = distances[neighbor_mask != 0]  # 填充0.0
    ...
    return distances  # distance的shape是(100, 9, 8)
