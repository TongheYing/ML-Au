...
]  # 当以from XX import *的形式导入类和函数时，只有在__all__变量中的类和函数才能被导入
...
class Atomwise(nn.Module):
    ...
    def __init__(
        ...
        stddev = torch.FloatTensor([1.0]) if stddev is None else stddev  # 平均值和标准差用来归一化输入数据

        # initialize single atom energies
        if atomref is not None:
            self.atomref = nn.Embedding.from_pretrained(
                torch.from_numpy(atomref.astype(np.float32))
            )  # 存储词嵌入向量 https://blog.csdn.net/qq_38883844/article/details/104331382
        else:
            self.atomref = None
        ...
        if outnet is None:  #
            ...
            )  # representation就是SchNet层，SchNet和MLP会连在一起
        else:
            self.out_net = outnet

        # build standardization layer
        self.standardize = schnetpack.nn.base.ScaleShift(mean, stddev)  # 反向归一化层

        # build aggregation layer
        if aggregation_mode == "sum":
            self.atom_pool = schnetpack.nn.base.Aggregate(axis=1, mean=False)  # 预测总能时，用sum
        elif aggregation_mode == "avg":
            self.atom_pool = schnetpack.nn.base.Aggregate(axis=1, mean=True)  # 预测分配到各原子的能量时，用avg
        else:
            raise AtomwiseError(
    ...
    def forward(self, inputs):
        ...
        yi = self.standardize(yi)  # 反向归一化，使预测值恢复到正常范围

        if self.atomref is not None:  # 减去基准值
            y0 = self.atomref(atomic_numbers)
            yi = yi + y0

        y = self.atom_pool(yi, atom_mask)  # 将单个原子的能量贡献累加
        ...
        if self.derivative is not None:
            ...
            result[self.derivative] = sign * dy  # 求导force
