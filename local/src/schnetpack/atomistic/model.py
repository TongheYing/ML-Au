...
class AtomisticModel(nn.Module):
    ...
    def __init__(self, representation, output_modules):
        ...
        # For gradients
        self.requires_dr = any([om.derivative for om in self.output_modules])  # 是否有网络要求了求导
        # For stress tensor
    ...
    def forward(self, inputs):
        ...
        """
        if self.requires_dr:
            inputs[Properties.R].requires_grad_()  # 原子坐标position的require_grad=True
        ...
        inputs["representation"] = self.representation(inputs)  # 这里是SchNet的关键层，返回的shape是(100, 9, 128)
        outs = {}
