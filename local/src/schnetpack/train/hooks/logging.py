...
class LoggingHook(Hook):
    ...
    def __init__(
        ...
        self._train_loss = 0
        # self._train_err_energy = 0
        # self._train_err_forces = 0
        self._counter = 0
    ...
    def on_batch_end(self, trainer, train_batch, result, loss):  # TODO 可以在此处理train_error
        if self.log_train_loss:
            n_samples = self._batch_size(result)
            self._train_loss += float(loss.data) * n_samples
            # self._train_err_energy += err_energy * n_samples
            # self._train_err_forces += err_forces * n_samples
...
class CSVHook(LoggingHook):
    ...
    def on_train_begin(self, trainer):
        ...
        if os.path.exists(self.log_path):
            ...
            with open(self.log_path, "r") as f:
                ...
                if len(lines) > 1:
                    ...
                    self._restart = True  # 从checkpoint读取数据，继续训练
                else:
                    remove_file = True
        ...
        if not self._restart:  # 给出日志文件log.csv的文件开头
            log = ""
            log += "Time"
            ...
            for i, metric in enumerate(self.metrics):
                # log += str(metric.name)
                if i < 2:
                    log += str(metric.name) + '-train'
                else:
                    log += str(metric.name) + '-validation'
                if i < len(self.metrics) - 1:
                    log += ","

            with open(self.log_path, "a+") as f:
                f.write(log + os.linesep)  # os.linesep给出当前操作系统的行终止符
    ...
    def on_validation_end(self, trainer, val_loss):
        if trainer.epoch % self.every_n_epochs == 0:
            ...
            if self.log_learning_rate:
                log += "," + str(trainer.optimizer.param_groups[0]["lr"])  # 如果有多个学习率
