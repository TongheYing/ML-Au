...
class SchNetInteraction(nn.Module):
    ...
    def __init__(
        ...
        self.cutoff_network = cutoff_network(cutoff)  # 初始化
        # interaction block
        self.cfconv = CFConv(
            n_atom_basis,
            n_filters,
...
class SchNet(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(max_z, n_atom_basis, padding_idx=0)  # max_z代表支持100种元素，
        # n_atom_basis是每种元素的编码长度  self.embedding的shape是(100, 128)

        # layer for computing interatomic distances
        self.distances = AtomDistances()  # 初始化计算原子间距离

        # layer for expanding interatomic distances in a basis
        if distance_expansion is None:  # 初始化高斯扩展距离
    ...
    def forward(self, inputs):
        ...
        x = self.embedding(atomic_numbers)  # 根据元素种类随机生成的atom_feature，128维度
        ...
        )  # 计算原子间距离，性质中本身就提供了相邻原子的index, 返回的r_ij的shape是(100, 9, 8)
        # expand interatomic distances (for example, Gaussian smearing)
        f_ij = self.distance_expansion(r_ij)  # cutoff是在高斯扩展中使用的
