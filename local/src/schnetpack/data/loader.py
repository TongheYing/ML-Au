...
def _collate_aseatoms(examples):
    ...
    if not has_neighbor_mask:  # neighbor_mask和neighbor相同尺寸
...
class AtomsLoader(DataLoader):
    ...
    def __init__(
        ...
        )  # 在上方的_collate_aseatoms中实现了collate_fn方法
    ...
    def get_statistics(
        ...
        with torch.no_grad():
            ...
            for row in self:  # 谁调用的该方法，self就指代那个对象，这里指AtomLoader类的self
    ...
    def _update_statistic(
        ...
        if single_atom_ref is not None:
            ...
            property_value -= p0
        if divide_by_atoms:
            property_value /= torch.sum(row["_atom_mask"], dim=1, keepdim=True)  # energy平均到了每个原子上
