...
def train_test_split(
    ...
    if split_file is not None and os.path.exists(split_file):
        ...
        num_train = num_train if num_train > 1 else num_train * len(data)  # 也可以用比例来划分数据集
        ...
        if stratify_partitions:
            ...
            idx = np.random.permutation(len(data))  # 随机打乱index
        ...
        if split_file is not None:
            ...
            )  # np.savez()函数将数据存储为.npz格式  https://blog.csdn.net/leilei7407/article/details/107511187

    train = data.create_subset(train_idx)  # 返回MD17()类
