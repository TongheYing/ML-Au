...
def get_center_of_mass(atoms):
    ...
    return np.dot(masses, atoms.arrays["positions"]) / masses.sum()  # 获得质心
...
class AtomsData(Dataset):
    ...
    def __init__(
        ...
        self.available_properties = self.get_available_properties(available_properties)  # 再次确认性质都存在于数据库中
        if load_only is None:
            self.load_only = self.available_properties
        if units is None:
            units = [1.0] * len(self.available_properties)  # units是用于乘的比例系数
    ...
    def get_available_properties(self, available_properties):
        ...
        with connect(self.dbpath) as conn:  # connect是python数据库的操作
            atmsrw = conn.get(1)
    ...
    def get_properties(self, idx):
        ...
        """
        idx = self._subset_index(idx)  # 若没有subset，则返回int:0
        with connect(self.dbpath) as conn:
            row = conn.get(idx + 1)
        at = row.toatoms()  # 从数据库中读取数据，转化为Atoms类，存储在程序中

        # extract properties
        properties = {}
        for pname in self.load_only:
            properties[pname] = torch.FloatTensor(row.data[pname].copy())  # 根据性质，读取数据库中的数据
...
def _convert_atoms(
    ...
    Returns:
        dict of torch.Tensor: Properties including neighbor lists and masks
            reformated into SchNetPack input format.  将ASE atoms对象转化为SchNet的input，多了neighbor等性质
    """
    if output is None:
        inputs = {}
    else:
    ...
    if centering_function:
        positions -= centering_function(atoms)  # 默认是get_center_of_mass，也就是将坐标平移到质心
    ...
    # If requested get neighbor lists for triples，可以在这里找到配对的原子对函数
    if collect_triples:
